...
uses GlobalConfig,GlobalConst,GlobalTypes,BCM2836,Platform{$IFNDEF CONSOLE_EARLY_INIT},PlatformRPi2{$ENDIF},Threads,HeapManager,Devices,SPI,I2C,DMA,PWM,GPIO,UART,Serial,MMC,BCMSDHOST,Framebuffer,Audio,SysUtils; 
...
const
 ...
 BCM2709_DMA_LITE_BURST_LENGTH = 2;              {Burst length for DMA Lite channels}
 BCM2709_DMA_NORMAL_BURST_LENGTH = 4;            {Burst length for normal channels}
 BCM2709_DMA_BULK_BURST_LENGTH = 16;             {Burst length for DMA Bulk channels}
...
function BCM2709BSCI2CRead(I2C:PI2CDevice;Address:Word;Buffer:Pointer;Size,Flags:LongWord;var Count:LongWord):LongWord;
function BCM2709BSCI2CWrite(I2C:PI2CDevice;Address:Word;Buffer:Pointer;Size,Flags:LongWord;var Count:LongWord):LongWord;
function BCM2709BSCI2CWriteRead(I2C:PI2CDevice;Address:Word;Initial:Pointer;Len:LongWord;Data:Pointer;Size,Flags:LongWord;var Count:LongWord):LongWord;
function BCM2709BSCI2CWriteWrite(I2C:PI2CDevice;Address:Word;Initial:Pointer;Len:LongWord;Data:Pointer;Size,Flags:LongWord;var Count:LongWord):LongWord;
...
function BCM2709I2CSlaveRead(I2C:PI2CDevice;Address:Word;Buffer:Pointer;Size,Flags:LongWord;var Count:LongWord):LongWord;
function BCM2709I2CSlaveWrite(I2C:PI2CDevice;Address:Word;Buffer:Pointer;Size,Flags:LongWord;var Count:LongWord):LongWord;
...
function BCM2709SDHCIHostSignal(SDHCI:PSDHCIHost;Semaphore:TSemaphoreHandle):LongWord;
...
function BCM2709FramebufferSetOffsetEx(Framebuffer:PFramebufferDevice;X,Y:LongWord;Pan,Switch:Boolean):LongWord;
...
begin
 ...
 {Initialize BCM2709SDHOST_FIQ_ENABLED}
 if not(FIQ_ENABLED) then BCM2709SDHOST_FIQ_ENABLED:=False;
  ...
  begin
   ...
   if BCM2709SDHCIHost <> nil then
    begin
     ...
     BCM2709SDHCIHost.SDHCI.HostSignal:=BCM2709SDHCIHostSignal;
     ...
     {BCM2709}
     BCM2709SDHCIHost.FIQ:=BCM2709SDHCI_FIQ_ENABLED;
      ...
      begin
       ...
       if BCM2709Framebuffer <> nil then
        begin
         ...
         BCM2709Framebuffer.Framebuffer.DeviceSetOffsetEx:=BCM2709FramebufferSetOffsetEx;
         ...
         {if EnvironmentGet('bcm2708_fb.fbswap') <> '1' then BCM2709Framebuffer.Framebuffer.Device.DeviceFlags:=BCM2709Framebuffer.Framebuffer.Device.DeviceFlags or FRAMEBUFFER_FLAG_SWAP;} {Handled by FramebufferAllocate}
...
begin
 ...
 if Rate > ((BCM2836_BSC_CLKT_TOUT_MASK * 1000) div BCM2709I2C_CLOCK_TIMEOUT) then
  begin
   Timeout:=BCM2836_BSC_CLKT_TOUT_MASK;
  end
 else
  begin
   Timeout:=BCM2709I2C_CLOCK_TIMEOUT * (Rate div 1000);
  end;
...
begin
 ...
 PBCM2709BSCI2CDevice(I2C).IgnoreNAK:=False;
...
begin
 ...
 {$IF DEFINED(BCM2709_DEBUG) or DEFINED(I2C_DEBUG)}
 if I2C_LOG_ENABLED then I2CLogDebug(I2C,'BCM2709: BSCI2C Read (Address=' + IntToHex(Address,4) + ' Size=' + IntToStr(Size) + ' Flags=' + IntToHex(Flags,8) + ')');
 {$ENDIF}
 ...
 if Size > 0 then
  begin
   ...
   PBCM2709BSCI2CDevice(I2C).Error:=False;
   PBCM2709BSCI2CDevice(I2C).IgnoreNAK:=(Flags and I2C_TRANSFER_IGNORE_NAK) <> 0;
...
begin
 ...
 {$IF DEFINED(BCM2709_DEBUG) or DEFINED(I2C_DEBUG)}
 if I2C_LOG_ENABLED then I2CLogDebug(I2C,'BCM2709: BSCI2C Write (Address=' + IntToHex(Address,4) + ' Size=' + IntToStr(Size) + ' Flags=' + IntToHex(Flags,8) + ')');
 {$ENDIF}
...
begin
 ...
 {$IF DEFINED(BCM2709_DEBUG) or DEFINED(I2C_DEBUG)}
 if I2C_LOG_ENABLED then I2CLogDebug(I2C,'BCM2709: BSCI2C Write Read (Address=' + IntToHex(Address,4) + ' Len=' + IntToStr(Len) + ' Size=' + IntToStr(Size) + ' Flags=' + IntToHex(Flags,8) + ')');
 {$ENDIF}
 ...
 if (Len > BCM2836_BSC_FIFO_SIZE) or not(BCM2709I2C_COMBINED_WRITEREAD) then
  begin
   ...
   {Write Initial}
   Result:=BCM2709BSCI2CWrite(I2C,Address,Initial,Len,Flags,Written);
   if Result = ERROR_SUCCESS then
    begin
     {Read Data}
     Result:=BCM2709BSCI2CRead(I2C,Address,Data,Size,Flags,Count);
    end;
  end
 else
  begin
...
begin
 ...
 {$IF DEFINED(BCM2709_DEBUG) or DEFINED(I2C_DEBUG)}
 if I2C_LOG_ENABLED then I2CLogDebug(I2C,'BCM2709: BSCI2C Write Write (Address=' + IntToHex(Address,4) + ' Len=' + IntToStr(Len) + ' Size=' + IntToStr(Size) + ' Flags=' + IntToHex(Flags,8) + ')');
 {$ENDIF}
    ...
    begin
     ...
     if (I2C <> nil) and (I2C.Data <> nil) then
      begin
       ...
       {Get Error}
       Error:=Status and (BCM2836_BSC_S_CLKT or BCM2836_BSC_S_ERR);
       if I2C.IgnoreNAK then Error:=Error and not(BCM2836_BSC_S_ERR);
...
begin
 ...
 {$IF DEFINED(BCM2709_DEBUG) or DEFINED(I2C_DEBUG)}
 if I2C_LOG_ENABLED then I2CLogDebug(I2C,'BCM2709: I2C Slave Read (Size=' + IntToStr(Size) + ' Flags=' + IntToHex(Flags,8) + ')');
 {$ENDIF}
...
begin
 ...
 {$IF DEFINED(BCM2709_DEBUG) or DEFINED(I2C_DEBUG)}
 if I2C_LOG_ENABLED then I2CLogDebug(I2C,'BCM2709: I2C Slave Write (Size=' + IntToStr(Size) + ' Flags=' + IntToHex(Flags,8) + ')');
 {$ENDIF}
...
begin
 ...
 if (Status and BCM2836_I2CSPI_MIS_RXMIS) <> 0 then 
  ...
  begin
   {$IF (DEFINED(BCM2709_DEBUG) or DEFINED(I2C_DEBUG)) and DEFINED(INTERRUPT_DEBUG)}
   if Status <> 0 then
    begin
     if I2C_LOG_ENABLED then I2CLogWarn(I2C,'BCM2709: I2C Slave Spurious Interrupt (MIS=' + IntToHex(Status,8) + ')');
    end;
   {$ENDIF}
     ...
     if Channel <> LongWord(INVALID_HANDLE_VALUE) then 
      begin
       ...
       PBCM2709DMAHost(DMA).Channels[Channel].Registers.CS:=BCM2836_DMA_CS_ACTIVE
                                                         or (BCM2836_DMA_CS_PRIORITY_MAX shl BCM2836_DMA_CS_PRIORITY_SHIFT)
                                                         or (BCM2836_DMA_CS_PRIORITY_MAX shl BCM2836_DMA_CS_PANIC_PRIORITY_SHIFT)
                                                         or BCM2836_DMA_CS_WAIT_FOR_OUTSTANDING_WRITES;
...
begin
 ...
 case Peripheral of
  ...
  DMA_DREQ_ID_SMI:Result:=BCM2836_DMA_DREQ_SMI;
 end;
end;
...
begin
 ...
 if not(Peripheral) then
  begin
   if Bulk then
    begin
     Burst:=(BCM2709_DMA_BULK_BURST_LENGTH shr Shift) - 1;
    end
   else if Lite then
    begin
     Burst:=(BCM2709_DMA_LITE_BURST_LENGTH shr Shift) - 1;
    end
   else
    begin
     Burst:=(BCM2709_DMA_NORMAL_BURST_LENGTH shr Shift) - 1;
    end;
   Block.TransferInformation:=Block.TransferInformation or (Burst shl BCM2836_DMA_TI_BURST_LENGTH_SHIFT);
  end; 
 {Wait Response}
 Block.TransferInformation:=Block.TransferInformation or BCM2836_DMA_TI_WAIT_RESP;
...
begin
 ...
 {Update SDHCI}
 {Driver Properties}
 if PBCM2709SDHCIHost(SDHCI).FIQ then
  begin
...
function BCM2709SDHCIHostSignal(SDHCI:PSDHCIHost;Semaphore:TSemaphoreHandle):LongWord;
{Implementation of SDHCIHostSignal API for BCM2709 SDHCI}
  ...
  begin
   ...
   if (InterruptMask and SDHCI_INT_CARD_INT) <> 0 then
    begin
     ...
     {Dispatch Interrupt}
     SDIOHostDispatchInterrupt(SDHCI,not(PBCM2709SDHCIHost(SDHCI).FIQ),PBCM2709SDHCIHost(SDHCI).FIQ);
    end;
...
function BCM2709FramebufferSetOffsetEx(Framebuffer:PFramebufferDevice;X,Y:LongWord;Pan,Switch:Boolean):LongWord;
{Implementation of FramebufferDeviceSetOffsetEx API for BCM2709 Framebuffer}
