...
begin
 ...
 try
  ...
  {$IFDEF IP_DEBUG}
  if NETWORK_LOG_ENABLED then NetworkLogDebug(nil,'IP: CheckHost');
  {$ENDIF}
...
begin
 ...
 try
  ...
  {$IFDEF IP_DEBUG}
  if NETWORK_LOG_ENABLED then NetworkLogDebug(nil,'IP: CheckRoute');
  {$ENDIF}
...
{Iterate though IP addresses in the address cache}
{Previous: The IP address returned by the previous call to this function}
{Lock: If True then lock the found entry before returning}
...
begin
 ...
 try
  ...
  {$IFDEF IP_DEBUG}
  if NETWORK_LOG_ENABLED then NetworkLogDebug(nil,'IP: CheckAddress');
  {$ENDIF}
