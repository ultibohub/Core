...
uses GlobalConfig,GlobalConst,GlobalTypes,BCM2838,Platform,{$IFDEF CPUARM}PlatformARM,PlatformARMv7,PlatformARMv7L,{$ENDIF CPUARM}{$IFDEF CPUAARCH64}PlatformAARCH64,PlatformARMv8,{$ENDIF CPUAARCH64}ARMGIC,HeapManager,Threads{$IFDEF CONSOLE_EARLY_INIT},Devices,Framebuffer,Console{$ENDIF}{$IFDEF LOGGING_EARLY_INIT},Logging{$ENDIF},SysUtils;
 ...
 if (Revision and BCM2838_BOARD_REVISION_ENCODED_FLAG) <> 0 then
  begin
   ...
   case (Revision and BCM2838_BOARD_REVISION_MODEL_MASK) of
    ...
    BCM2838_BOARD_REVISION_MODEL_CM4:begin
  ...
  begin
   ...
   if RPi4Framebuffer <> nil then
    begin
     ...
     RPi4Framebuffer.Framebuffer.DeviceSetProperties:=RPi4FramebufferDeviceSetProperties;
     {Driver}
     ...
     {if SysUtils.GetEnvironmentVariable('bcm2708_fb.fbswap') <> '1' then RPi4Framebuffer.Framebuffer.Device.DeviceFlags:=RPi4Framebuffer.Framebuffer.Device.DeviceFlags or FRAMEBUFFER_FLAG_SWAP;} {Handled by FramebufferAllocate}
...
begin
 ...
 try
  ...
  Tag.Header.Tag:=BCM2838_MBOX_TAG_TST_VSYNC;
