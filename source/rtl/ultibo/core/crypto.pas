...
=======
 ...
 Zlib - crc32.c - Copyright (C) 1995-2022, Mark Adler
...
( $0000000000000000, $0000001000000000, $0000000040000000, $0000001040000000,
 ...
 CRC32_POLY = LongWord($EDB88320); {0,1,2,4,5,7,8,10,11,12,16,22,23,26}

 {$IFDEF CRYPTO_CRC_PRECOMPUTED_TABLES}
 CRC32_TABLE:array[0..255] of LongWord = (
  ...
  $2d02ef8d);
  {Precomputed table for CRC32 values}
 {$ENDIF}
...
function CRC32(CRC:LongWord;Data:PByte;Size:LongWord):LongWord;
...
begin
 ...
 {Init CRC32 Tables}
 {CRC32InitTable;} {Done on first use}
...
function CRC32(CRC:LongWord;Data:PByte;Size:LongWord):LongWord;
{Computer a CRC32 on the supplied data up to the specified size}
{CRC: The current CRC value from previous calls}
{Data: The start of the data buffer to compute the CRC for}
{Size: The size of the data buffer to compute the CRC for}
{Return: The updated CRC value}
{The first call should pass nil for the data and 0 for the size to initialize the CRC}
begin
 {}
 Result:=0;
 ...
 if not CRC32Initialized then
  begin
   CRC32InitTable;
   if not CRC32Initialized then Exit;
  end;

 CRC:=CRC xor $FFFFFFFF;
  while (Size >= 4) do
  begin
    CRC:=CRC32Table[(CRC xor Data[0]) and $FF] xor (CRC shr 8);
   ...
   for Count:=0 to 7 do
    begin
     if (Value and 1) <> 0 then
      Value:=(Value shr 1) xor CRC32_POLY
     else
      Value:=(Value shr 1);
    end;
