...
var
 ...
 InterruptHandlers:array[0..(BCM2835_IRQ_COUNT - 1)] of TInterruptHandler;
 InterruptExHandlers:array[0..(BCM2835_IRQ_COUNT - 1)] of TInterruptExHandler;
 InterruptParameters:array[0..(BCM2835_IRQ_COUNT - 1)] of Pointer; 
...
function RPiRequestIRQ(CPUID,Number:LongWord;Handler:TInterruptHandler;Parameter:Pointer):LongWord;
function RPiReleaseIRQ(CPUID,Number:LongWord;Handler:TInterruptHandler;Parameter:Pointer):LongWord;
...
function RPiRequestFIQ(CPUID,Number:LongWord;Handler:TInterruptHandler;Parameter:Pointer):LongWord; 
function RPiReleaseFIQ(CPUID,Number:LongWord;Handler:TInterruptHandler;Parameter:Pointer):LongWord;
...
begin
 ...
 RequestIRQHandler:=RPiRequestIRQ;
 ReleaseIRQHandler:=RPiReleaseIRQ;
 RequestExIRQHandler:=RPiRequestExIRQ;
 ReleaseExIRQHandler:=RPiReleaseExIRQ;

 {Register Platform FIQ Handlers}
 RequestFIQHandler:=RPiRequestFIQ;
 ReleaseFIQHandler:=RPiReleaseFIQ;
 ...
 if (Revision and BCM2835_BOARD_REVISION_ENCODED_FLAG) <> 0 then
  begin
   ...
   case (Revision and BCM2835_BOARD_REVISION_MODEL_MASK) of
    ...
    BCM2835_BOARD_REVISION_MODEL_B,BCM2835_BOARD_REVISION_MODEL_BPLUS,BCM2835_BOARD_REVISION_MODEL_COMPUTE,BCM2835_BOARD_REVISION_MODEL_ZERO:begin
      {Get Memory Base/Size}
      MEMORY_BASE:=$00000000;
  ...
  begin 
   {Old Style Revision}
   case (Revision and BCM2835_BOARD_REV_MASK) of
    BCM2835_BOARD_REV_A_7,BCM2835_BOARD_REV_A_8,BCM2835_BOARD_REV_A_9,BCM2835_BOARD_REV_A_PLUS,BCM2835_BOARD_REV_A_PLUS_2:begin
      {Get Memory Base/Size}
      MEMORY_BASE:=$00000000;
      MEMORY_SIZE:=SIZE_256M;
      {Get Memory Page Size}
    ...
    BCM2835_BOARD_REV_CM,BCM2835_BOARD_REV_CM_2,BCM2835_BOARD_REV_ZERO:begin
      {Get Memory Base/Size}
      MEMORY_BASE:=$00000000;
      MEMORY_SIZE:=SIZE_512M;
      {Get Memory Page Size}
...
begin
 ...
 BCM2708FRAMEBUFFER_CACHED:=False;
...
begin
 ...
 case BOARD_TYPE of
  BOARD_TYPE_RPIA,BOARD_TYPE_RPIB:begin
    ...
    GPIOFunctionSelect(RPI_GPIO_ACTLED_GPFSEL,Value);
   end;
  BOARD_TYPE_RPIA_PLUS,BOARD_TYPE_RPIB_PLUS,BOARD_TYPE_RPI_ZERO:begin
    {Read current value of GPFSEL}
    ...
    {Write new value to GPFSEL} 
    GPIOFunctionSelect(RPIPLUS_GPIO_ACTLED_GPFSEL,Value);
   end;
 end;
end;
...
begin
 ...
 case BOARD_TYPE of
  BOARD_TYPE_RPIA,BOARD_TYPE_RPIB:begin
    {LED On}
    GPIOOutputClear(RPI_GPIO_ACTLED_GPCLR,(RPI_GPIO_ACTLED_GPMASK shl RPI_GPIO_ACTLED_GPSHIFT));
   end;
  BOARD_TYPE_RPIA_PLUS,BOARD_TYPE_RPIB_PLUS,BOARD_TYPE_RPI_ZERO:begin
    {LED On}
    GPIOOutputSet(RPIPLUS_GPIO_ACTLED_GPSET,(RPIPLUS_GPIO_ACTLED_GPMASK shl RPIPLUS_GPIO_ACTLED_GPSHIFT));
   end;
 end;
end;
...
begin
 ...
 case BOARD_TYPE of
  BOARD_TYPE_RPIA,BOARD_TYPE_RPIB:begin
    {LED Off}
    GPIOOutputSet(RPI_GPIO_ACTLED_GPSET,(RPI_GPIO_ACTLED_GPMASK shl RPI_GPIO_ACTLED_GPSHIFT));
   end;
  BOARD_TYPE_RPIA_PLUS,BOARD_TYPE_RPIB_PLUS,BOARD_TYPE_RPI_ZERO:begin
    {LED Off}
    GPIOOutputClear(RPIPLUS_GPIO_ACTLED_GPCLR,(RPIPLUS_GPIO_ACTLED_GPMASK shl RPIPLUS_GPIO_ACTLED_GPSHIFT));
   end;
 end;
end;
...
begin
 {}
 Result:=RPiRequestExIRQ(CPUID,Number,Handler,nil,Parameter);
end;
...
begin
 {}
 Result:=RPiReleaseExIRQ(CPUID,Number,Handler,nil,Parameter);
end;
...
begin
 {}
 Result:=RPiRequestExFIQ(CPUID,Number,Handler,nil,Parameter);
end;
...
begin
 {}
 Result:=RPiReleaseExFIQ(CPUID,Number,Handler,nil,Parameter);
end;
...
begin
 ...
 try 
  ...
  WatchdogRegisters.RSTS:=BCM2835_PM_PASSWORD or (Current and BCM2835_PM_RSTC_WRCFG_CLR) or BCM2835_PM_RSTS_HADWRH_SET;
